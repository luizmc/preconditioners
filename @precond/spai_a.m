...
% Input:
% A    (required) : N x N coefficient matrix.
% NMAX (optional) : max number of nonzero on each column of ainv.
% TOL  (optional) : acceptable tolerance to ||ej-Amj||
% INNZ (optional) : specifies how many nonzero elements will be added to
%                   ainv at each inner loop. The number of inner loops is
%                   given by floor(NMAX/INNZ). Its default value is INNZ=1.
...
if(nargin<3 || isempty(tol))
    tol=1E-1;                   % max value of ||ej - A*mj||
end
if(nargin<4 || isempty(innz))
    innz=1;
end

Mjnnz_max=nmax;           % max number of nonzero entries in Mj
Mi=zeros(n*Mjnnz_max,1);  % row index of M
Mj=Mi;                    % col index of M
Ms=Mi;                    % values of M
Mnnz=0;                   % total number of nonzero elements in M
iloop=floor(nmax/innz);   % number of inner loop
...
for j = 1:n
    ...
    [Q,R]=qr(ss_A);
    ss_mj=R\(Q'*ss_ej);
    for i=1:iloop % improves the sparsity pattern
